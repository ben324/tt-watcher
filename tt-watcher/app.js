...
function setPoint(lat, lng, address) {
  ...
  $("s-lat").value = lat.toFixed(5); $("s-lng").value = lng.toFixed(5);
  $("s-coords").textContent = lat.toFixed(5) + ", " + lng.toFixed(5) + "  ·  " + currentRadiusMiles() + " mi";
  if (address) $("s-address").value = address;
  if (!map) return;
  const here = [lat, lng];
...
function initMap() {
  ...
  map.on("click", async (ev) => {
    $("search-error").textContent = ""; setPoint(ev.latlng.lat, ev.latlng.lng);
    try { const name = await reverseGeocode(ev.latlng.lat, ev.latlng.lng); if (name) $("s-address").value = name; } catch (_) {}
  });
...
$("s-radius").oninput = () => {
  $("s-radius-val").textContent = val("s-radius") || "25";
...
$("geocode").onclick = async () => {
  ...
  if (!q) { $("search-error").textContent = "Type an address first"; return; }
  try { const hit = await geocodeAddress(q); setPoint(hit.lat, hit.lng, hit.label); }
  catch (err) { $("search-error").textContent = err.message; }
};
$("s-address").addEventListener("keydown", (e) => { if (e.key === "Enter") { e.preventDefault(); $("geocode").click(); } });
$("geo").onclick = () => {
  $("search-error").textContent = "";
  ...
  navigator.geolocation.getCurrentPosition(async (pos) => {
    ...
    try { const name = await reverseGeocode(pos.coords.latitude, pos.coords.longitude); if (name) $("s-address").value = name; } catch (_) {}
  }, () => { $("search-error").textContent = "Could not read location"; });
};
$("search-form").onsubmit = async (e) => {
  e.preventDefault(); $("search-error").textContent = "";
...
$("event-form").onsubmit = async (e) => {
  ...
  try {
    ...
    const idBox = $("e-id"); if (idBox) idBox.value = "";
    await loadWatches();
...
async function loadWatches() {
  const box = $("watches"); $("list-error").textContent = "";
  try {
...
async function bootDesk() { await loadOptions(); await loadWatches(); initMap(); }
